...
This package installs three executables.  The two that users are likely to run directly are:
...
* pecnv is a C++ program implementing our various methods via subcommands.  Most of the subcommands are run by the pecnv.sh script.  However, users interested in detecting transposable element mutations will want to look at the "teclust" subcommand, which implements the TE detection method of Cridland _et al._ (2013) PMID 23883524.  This module is explained in more detail below.

#Using the pecnv program

Many users will just want to run the pecnv.sh master script.  For more on that, see the next sections.
...
Each subcommand has its own detailed help info, where applicable.  This info will be printed to stderr if no options are given.
...
* The -u/--ulimit option allows the user to provide a hard RAM limit to the step where the process subcommand reads the BAM file.  For complex genomes with a large number of repetitively-mapping reads, the RAM usage may get quite high.  Thus, this option is provided so that the process may be killed rather than taking down the user's system.  Note that, if you use this option, you may see bizarre errors reported to stderr.  It is unlikely that these errors are actual segfaults, etc., in the program.  Rather, they are the outcome of what happens when a kill signal is sent by the system and not handled directly by the affected program.  In practice, the sorts of signals sent by ulimit violations are not always handleable, and thus the program makes no attempt to do so.
...
2. The process subcommand reads the resulting BAM file, and collects reads in unusual mapping orientations, writing data to several output files.
3. The mdist subcommand estimates the insert size distribution from the BAM file.  This is done separately from step 2 to minimize RAM use.  Also, power users can modify the work flow to separate these tasks out on a cluster.
4. Rscript is invoked to get the 99.9th quantile of the insert size distribution
5. The cnvclust subcommand clusters the divergent, parallel, and unlinked read pairs into putative CNV calls.  The output files are described below.
...
The process subcommand creates the following files:
...
The Cridland et al. method is implemented in the subcommand teclust, with some additional features.  The program is able to make use of TE annotations from an existing reference genome, if they are available.
...
pecnv teclust -u umuFile -m ummFile -o outfile.gz -i `pecnv_insert_qtile mdistfile 0.99`
```
...
```
pecnv teclust -u umuFile -m ummFile -o outfile.gz -i `pecnv_insert_qtile mdistfile 0.99` -t tefile --ummHitTE
```
...
pecnv teclust -u umuFile -m ummFile -o outfile.gz -i `pecnv_insert_qtile mdistfile 0.99` -t tefile
...
```
pecnv teclust -u umuFile -m ummFile -o outfile.gz -i `pecnv_insert_qtile mdistfile 0.99` -t tefile -b bamfile
```
...
```
pecnv teclust -u umuFile -m ummFile -o outfile.gz -i `pecnv_insert_qtile mdistfile 0.99` -t tefile -b bamfile --phrapdir phrapdir
```
...
#Clustering of "orphaned" reads from paired-end sequencing

The "orphan" module scans a BAM file generated by the alignment of pared-end data to a reference genome.  The module looks for "orphans", which we defined as mapped reads with unmapped partners.

The usage is simple:

```
#Write results to a gzipped BED file.  The -s option is optional.
pecnv orphan -b bamfile -o output.bed.gz -s samplename

#Write results to stdout
pecnv orphan -b bamfile -o stdout -s samplename
```

The output will be a 6-column BED format. The score is the mapping quality of the read. The name column is a concatenation of the sample ID label and the read name prefix.

The module does not filter on mapping quality, read uniqueness, or anything else.  The user may filter on mapping quality based on the output.  

##Clustering of "orphans"

Example usage:

```
#Run pecnv orphan and print results to stdout
pecnv orphan -b bamfile.bam -o stdout  -s MYSAMPLE | \ 
#Filter mapping quality > 0
     awk '{if($5 >0) print $0}' | \
#Use bedtools to merge based on strand and distance.  The output score is mean mapping quality,
#And the number of reads per cluster will be reported
     bedtools merge -s -n -scores mean -d DISTANCE -i - | \
#Require at least 4 reads per cluster
     awk '{if ($4 > 3) print $0}' | \
#Sort on chromosome and positon. Write to output file
     sort -k1,1 -k2,2n > MYSAMPLE.orphans.clustered
```